...
bool update_frame(int& fd, i2c_frame& frame){
	...
	// add buffer to store and read i2c data
	uint8_t buffer[22];
	// read data
	// auto start = std::chrono::steady_clock::now();
	ssize_t bytes_read = read(fd, buffer, 22); // replace with i2c_smbus
	...
	// while(bytes_read < 22){
	// 	if(std::chrono::duration_cast<std::chrono::milliseconds>(now-start).count() > PX4FLOW_TIMEOUT){
...
int main(int argc, char** argv){
	...
	// PX4Flow* flow = new PX4Flow();  // init px4 flow sensor wrapper class
	i2c_frame frame;
	int fd = read_port();
	while(true){
		if(fd < 0){
